...
#define MAVLINK_MSG_ID_PIKSI_GLO_BIASES_LEN 14
#define MAVLINK_MSG_ID_PIKSI_GLO_BIASES_MIN_LEN 14
#define MAVLINK_MSG_ID_186_LEN 14
#define MAVLINK_MSG_ID_186_MIN_LEN 14

#define MAVLINK_MSG_ID_PIKSI_GLO_BIASES_CRC 210
#define MAVLINK_MSG_ID_186_CRC 210
...
#define MAVLINK_MESSAGE_INFO_PIKSI_GLO_BIASES { \
    ...
    {  { "sender_id", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_piksi_glo_biases_t, sender_id) }, \
         ...
         { "len", NULL, MAVLINK_TYPE_UINT8_T, 0, 4, offsetof(mavlink_piksi_glo_biases_t, len) }, \
         { "data", NULL, MAVLINK_TYPE_UINT8_T, 9, 5, offsetof(mavlink_piksi_glo_biases_t, data) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_PIKSI_GLO_BIASES { \
                               ...
                               uint16_t sender_id, uint16_t msg_type, uint8_t len, const uint8_t *data)
{
    ...
    _mav_put_uint8_t_array(buf, 5, data, 9);
...
{
    return mavlink_msg_piksi_glo_biases_pack(system_id, component_id, msg, piksi_glo_biases->sender_id, piksi_glo_biases->msg_type, piksi_glo_biases->len, piksi_glo_biases->data);
}
...
{
    return mavlink_msg_piksi_glo_biases_pack_chan(system_id, component_id, chan, msg, piksi_glo_biases->sender_id, piksi_glo_biases->msg_type, piksi_glo_biases->len, piksi_glo_biases->data);
}
...
static inline void mavlink_msg_piksi_glo_biases_send(mavlink_channel_t chan, uint16_t sender_id, uint16_t msg_type, uint8_t len, const uint8_t *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_PIKSI_GLO_BIASES_LEN];
    _mav_put_uint16_t(buf, 0, sender_id);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_piksi_glo_biases_send(chan, piksi_glo_biases->sender_id, piksi_glo_biases->msg_type, piksi_glo_biases->len, piksi_glo_biases->data);
#else
 ...
 */
static inline void mavlink_msg_piksi_glo_biases_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint16_t sender_id, uint16_t msg_type, uint8_t len, const uint8_t *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
{
    return _MAV_RETURN_uint8_t_array(msg, data, 9,  5);
}

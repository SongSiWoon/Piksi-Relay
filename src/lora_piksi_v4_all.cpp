// v4: overflow + mutex 모두 수정
//   [수정] obs_callback: safe_len으로 overflow 방지
//   [수정] write_to_lora: mutex 범위 축소 + TOCTOU 수정
//
// 로그 파일: 실행 시 piksi_v4_all_YYYYMMDD_HHMMSS.log 자동 생성
//
// 빌드:
...
//   g++ lora_piksi_v4_all.cpp sbp.o edc.o -I ../include/lora_mavlink/swarm -I ../include/libsbp/c/include -o piksi_relay_v4 -pthread
// 실행:
//   ./piksi_relay_v4 -d /dev/ttyACM1 -l /dev/ttyUSB0
...
// 전역 통계 카운터
// ─────────────────────────────────────────────
static uint64_t g_obs_total       = 0;
static uint64_t g_obs_overflow    = 0;
static uint64_t g_obs_corruption  = 0;
static int      g_max_len_seen    = 0;
static int      g_max_sats_seen   = 0;
...
void log_printf(const char* fmt, ...)
{
    ...
    printf("[%s.%03ld] ", ts, tv.tv_usec / 1000);
    va_start(args, fmt);
    vprintf(fmt, args);
    va_end(args);

    if (g_log_fp) {
        fprintf(g_log_fp, "[%s.%03ld] ", ts, tv.tv_usec / 1000);
        va_start(args, fmt);
        vfprintf(g_log_fp, fmt, args);
        va_end(args);
        fflush(g_log_fp);  // 크래시 나도 데이터 유실 방지
    }
}
...
{
    log_printf("[CRASH] signal %d received!\n", sig);
    log_printf("[CRASH STATS] obs_total=%llu  overflow=%llu  corruption=%llu  max_len=%d  max_sats=%d\n",
               ...
               g_max_sats_seen);
    fflush(stdout);
    if (g_log_fp) { fflush(g_log_fp); fclose(g_log_fp); }
    _exit(1);
}
...
void obs_callback(u16 sender_id, u8 len, u8 msg[], void *context)
{
    ...
    log_printf("MSG_OBS 0x%0X sats=%d len=%d\n", sender_id, obs_in_msg, len);
    ...
    if (len > OBS_DATA_SIZE) {
        g_obs_overflow++;
        log_printf("[OVERFLOW?] len=%d > data[%d]  sats=%d  overflow=%d bytes"
    ...
    for (int i = 0; i < CANARY_SIZE; i++) {
        if (guarded.canary[i] != CANARY_PATTERN) {
            if (!corrupted) {
                g_obs_corruption++;
                log_printf("[CORRUPTION] canary[%d]=0x%02X (0x%02X expected!) "
    ...
    if (!corrupted && len > OBS_DATA_SIZE) {
        log_printf("[OVERFLOW?] overflow 조건이지만 canary 안 깨짐 (컴파일러 패딩 의심)\n");
    }
...
{
    log_printf("========== [STATS] ==========\n");
    log_printf("  OBS 수신 총계    : %llu\n",  (unsigned long long)g_obs_total);
    log_printf("  OVERFLOW 발생    : %llu  (len > 164)\n", (unsigned long long)g_obs_overflow);
    log_printf("  CORRUPTION 감지  : %llu  (canary 오염)\n", (unsigned long long)g_obs_corruption);
    log_printf("  최대 len         : %d bytes\n", g_max_len_seen);
    log_printf("  최대 위성 수     : %d개\n",     g_max_sats_seen);
    if (g_obs_total > 0)
        log_printf("  overflow 비율    : %.1f%%\n",
...
void write_to_lora()
{
    ...
    while (1) {
        ...
        // [v4 수정] mutex는 queue pop까지만, write/tcdrain은 락 밖에서
        if (now_t - before_t > 100) {
            mavlink_message_t message;
            bool has_msg = false;
            {
                std::lock_guard<std::mutex> lock(mtx);
                if (!_mavlink_que.empty()) {
...
int main(int argc, char **argv)
{
    ...
    // 로그 파일 생성 (실행 시각으로 파일명 결정)
    {
        time_t now = time(NULL);
        struct tm* t = localtime(&now);
        char logname[64];
        strftime(logname, sizeof(logname), "piksi_v4_all_%Y%m%d_%H%M%S.log", t);
    ...
    log_printf("=== lora_piksi v4: all fixes ===\n");
